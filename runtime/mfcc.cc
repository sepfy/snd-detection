...
public:
    ...
    int processBuffer(int16_t *inputs, int size) {
        ...
        memcpy(buffer, inputs, bufferLength);
        for (int i=0; i<bufferLength; i++)
            prevsamples[i] = buffer[i];
        delete [] buffer;
        ...
        bufferLength = frameShiftSamples;
        buffer = new int16_t[bufferLength];
        memcpy(buffer, inputs + bufferLength, bufferLength);
        //printf("%d\n", bufferLength);
        // Read data and process each frame
        //
        for(int i = 2; i < 150; i++) {
          ...
          memcpy(buffer, inputs + bufferLength*i, bufferLength);
        }
        //printf("%d\n", mfcc.size()-1);
